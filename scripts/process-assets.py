...
DRIVE = BASE_DIR / "drive-assets" / "AMC SOLUCIONES PERU"
...
def process_model(model_dir: str, slug: str, is_new=False):
    ...
    for vid_file in sorted(src_dir.glob("*.mp4")) + sorted(src_dir.glob("*.mov")):
        mp4_name = f"video-{len(list(dest_dir.glob('video-*')))+1:02d}.mp4"
...
# Carousel map skipped because we don't have the /tmp/port_* images
...
# Also create amc-8100.webp from its first product image
...
if src_8100.exists():
    ...
    left = (img.width - min_dim) // 2
    top = (img.height - min_dim) // 2
    img = img.crop((left, top, left + min_dim, top + min_dim)).resize((800, 800), Image.LANCZOS)
    (PUBLIC / "amc-8100.webp").unlink(missing_ok=True)
    img.save(PUBLIC / "amc-8100.webp", 'WEBP', quality=90)

# Also create amc-cm3400.webp hero image from perspective photo
src_cm3400 = DRIVE / "AMC-CM3400" / "4c811b33-38c7-47b8-b72a-551f8ac9ecc7.jpg"
if src_cm3400.exists():
    img = Image.open(src_cm3400)
    if img.mode != 'RGB': img = img.convert('RGB')
    min_dim = min(img.size)
    left = (img.width - min_dim) // 2
    top = (img.height - min_dim) // 2
    img = img.crop((left, top, left + min_dim, top + min_dim)).resize((800, 800), Image.LANCZOS)
    (PUBLIC / "amc-cm3400.webp").unlink(missing_ok=True)
    img.save(PUBLIC / "amc-cm3400.webp", 'WEBP', quality=90)
